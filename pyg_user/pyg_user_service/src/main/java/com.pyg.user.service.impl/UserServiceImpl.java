...
public class UserServiceImpl implements UserService {
	...
	public PygResult add(TbUser user, String smsCode) {
		...
		try {
			// 从redis中获取smsCode
			String code = redisTemplate.boundHashOps("smsCode").get(user.getPhone()).toString();
			// 判断验证码是否正确
	...
	public PygResult smsCode(String phone) {
		try {
			...
			numberMap.put("code",number+"");

			mapMessage.put("number", JSON.toJSONString(numberMap));

			// 给短信发送网管服务发送消息pyg-sms
